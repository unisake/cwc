...
/* TODO: core.cに分離する DEソケットのコマンド受付ハンドラ */
static int handle_de_socket(int fd, uint32_t mask, void *data) {
    struct tinywl_server *server = data;
    (void)server; // 未使用警告を抑制
    // クライアントの接続を受け付ける
    ...
    if (len > 0) {
        buf[len] = '\0';
        wlr_log(WLR_INFO, "DE command: %s", buf);
		char cmd[16];
		char response[1024];
		sscanf(buf, "%s", cmd);  // まずコマンド名だけ取り出す

		if (strcmp(cmd, "wm") == 0) {
			int x, y, w, h; char id[256];
			if (sscanf(buf, "%*s [%d,%d,%d,%d] %s", &x, &y, &w, &h, id) == 5) {
				snprintf(response, sizeof(response), "wm: x=%d y=%d w=%d h=%d id=%s\n", x, y, w, h, id);
				write(client, response, strlen(response));
			}
		} else if (strcmp(cmd, "km") == 0) {
			// kmは別フォーマットでパース
			snprintf(response, sizeof(response), "km\n");
			write(client, response, strlen(response));
		} else if (strcmp(cmd, "pm") == 0) {
			// pmも別フォーマットでパース
			snprintf(response, sizeof(response), "pm\n");
			write(client, response, strlen(response));
		} else {
			snprintf(response, sizeof(response), "parse error\n");
			write(client, response, strlen(response));
		}
    }
...
int main(int argc, char *argv[]) {
	...
	struct tinywl_server server = {0};//ソケット構造体を空に

	/* The Wayland display is managed by libwayland. It handles accepting
	 * clients from the Unix socket, managing Wayland globals, and so on.
	 *
	 * Wlディスプレイはlibwaylandで管理される。この操作は
	 * Unixソケットにクライアントを受け入れてもらう、
	 * 管轄をWaylandに移行する、など。*/
	 ...
	 *
	 * このバックエンドはwlrootがどのようなハードウェアであるかを認識します。
	 * そして環境に合わせて自動で最適なバックエンド処理を作ってくれます*/
	 ...
	 *
	 * 自動でレンダラをPixman,GLES2,Vulkanのどれかに合わせて作成します。
	 * ユーザはWLR_RENDERERという環境変数で特定できます。
	 * */
...
/* TODO: core.cに分離する DEソケットの初期化 */
	mkdir("/run/de", 0755);
	const char *de_sock_path = "/run/de/core.sock";
	unlink(de_sock_path); // 古いソケットファイルを削除

	int de_fd = socket(AF_UNIX, SOCK_STREAM, 0);
	server.de_sock_fd = de_fd;
	if (server.de_sock_fd < 0) {
		wlr_log(WLR_ERROR, "failed to create DE socket");
		return 1;
	}

	struct sockaddr_un addr = {.sun_family = AF_UNIX};
	strcpy(addr.sun_path, de_sock_path);
	bind(server.de_sock_fd, (struct sockaddr *)&addr, sizeof(addr));
	listen(server.de_sock_fd, 5);

	struct wl_event_loop *loop = wl_display_get_event_loop(server.wl_display);
	wl_event_loop_add_fd(loop, server.de_sock_fd, WL_EVENT_READABLE, handle_de_socket, &server);
